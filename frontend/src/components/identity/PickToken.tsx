        ...
        <div>
            <div className="w-full flex justify-center">
                <button
                    ...
                    className={`flex items-center font-medium py-2 px-3 rounded-lg ${tokenIdx !== null ? "bg-slate-100 drop-shadow-xl" : " bg-blue-600 text-white whitespace-nowrap"}`}
                >
                                        ...
                                        <Combobox value={tokenIdx} onChange={(id) => { setTokenIdx(id); setShowSearchModal(false); callback(tokens[id]); }} >
                                            ...
                                            <Combobox.Options className="mt-2 h-[40vh] overflow-auto ">
                                                {searchResults.length === 0 && search !== '' ? (
                                                    ...
                                                    return (
                                                        <>
                                                            <Combobox.Option className={({ active }) => `flex items-center gap-5 p-4 cursor-pointer rounded-lg ${active ? 'bg-gray-200' : 'text-gray-900'}`} key={token.id} value={token.id}>
